...
import {
  ...
  getMenuManagerList,
  getMealPlanAnalysis,
  createProcurementCartFromMealPlan,
...
function pickRotatingMenu(items: MenuManagerItem[], usedMenuIds: Set<string>, seed: number) {
  if (items.length === 0) {
    return null
  }
...
function pickAutoMealMenus(
  ...
  addMenuIfPresent(
    mealMenus,
    preferredMainMenu ?? pickRotatingMenu(primaryMainMenus.length > 0 ? primaryMainMenus : mainMenus, usedMenuIds, seed),
    usedMenuIds,
  )

  if (foodType === AUTO_KOREAN_FOOD_TYPE) {
    addMenuIfPresent(mealMenus, pickRotatingMenu(riceMenus, usedMenuIds, seed + 10), usedMenuIds)
  }

  addMenuIfPresent(mealMenus, pickRotatingMenu(soupMenus, usedMenuIds, seed + 1), usedMenuIds)

  Array.from({ length: 2 }).forEach((_, sideIndex) => {
    addMenuIfPresent(mealMenus, pickRotatingMenu(normalSideMenus, usedMenuIds, seed + sideIndex + 2), usedMenuIds)
  })
...
function TableMealsManager() {
  ...
  const [isProcurementCreating, setIsProcurementCreating] = useState(false)
    ...
    const loadShortageMenus = async () => {
      ...
      const entries = await Promise.all(
        displayedMenuIds.map(async (menuId) => {
          try {
            const items = await getAccountMenuDetailList(menuId)
  ...
  useEffect(() => {
    ...
    const loadDetailList = async () => {
      ...
      try {
        ...
        setDetailError('')
        const items = await getMenuDetailList(selectedDetailMenu.menu_id)
        if (!isMounted) {
          return
        }
  ...
  const createShortageOrder = async (menuId: string) => {
    if (!selectedAccountId || !selectedTableMeal?.table_id || isProcurementCreating) return
    setIsProcurementCreating(true)
    try {
      const result = await createProcurementCartFromMealPlan(selectedAccountId, selectedTableMeal.table_id)
      if (result.item_count < 1) {
        setAlert({ type: 'success', title: '발주 확인', message: '현재 발주가 필요한 부족 식자재가 없습니다.' })
        return
      }
      navigate(
        `/order_manager/food_order/${encodeURIComponent(menuId)}?procurement_cart_id=${encodeURIComponent(String(result.procurement_cart_id))}`,
        { state: { procurementCartId: result.procurement_cart_id, procurementItemCount: result.item_count } },
      )
    } catch (error) {
      setAlert({
        type: 'error',
        title: '발주 카트 생성 실패',
        message: error instanceof Error ? error.message : '부족 식자재를 발주 카트에 담지 못했습니다.',
      })
    } finally {
      setIsProcurementCreating(false)
    }
  }
              ...
              {!isDetailLoading && !detailError && detailItems.length > 0 ? (
                ...
                {detailItems.some((item) => getIngredientStockStatus(item).needsOrder) ? (
                  <button
                    ...
                    onClick={() => void createShortageOrder(selectedDetailMenu.menu_id)}
                    disabled={isProcurementCreating}
                  >
                    {isProcurementCreating ? '발주 카트 생성 중' : '부족 식자재 발주하기'}
                  </button>
                ) : null}
                </>
              ) : null}
